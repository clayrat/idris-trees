...
interface Eq1 (f : Nat -> Type) where
  eq1 : f n -> f n -> Bool
...
interface Monad1 (t : Type) (m : (t -> Type) -> t -> Type) | m where
  pure1 : f a -> m f a
  (>>>=) : m f a -> ({b : t} -> f b -> m g b) -> m g a  

Monad1 Nat Box where
    pure1 = BoxZ
    (>>>=) {a} (BoxZ x) y = y {b=a} x 
    (>>>=) (BoxS x) y = BoxS (x >>>= y)

-- product functor
data ProdF : (f : Nat -> Type) -> (g : Nat -> Type) -> (a : Nat) -> Type where
  MkProdF : f a -> g a -> ProdF f g a

-- functor composition
data CompF : (f : Type -> Type) -> (g : Nat -> Type) -> (a : Nat) -> Type where
